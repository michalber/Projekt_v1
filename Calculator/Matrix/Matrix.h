...
public:
	Matrix(int height_t, int width_t);
	...
	Matrix add(T const &value);
	Matrix subtract(T const &value);
	Matrix multiply(T const &value);

	Matrix add(Matrix<T> const &m) const;
	Matrix subtract(Matrix<T> const &m) const;
	Matrix multiply(Matrix<T> const &m) const;
	Matrix dot(Matrix<T> const &m) const;
...
template <class T>
Matrix<T>::Matrix(int height_f, int width_f):height(height_f),width(width_f)
{
	//this->height = height;
	//this->width = width;
